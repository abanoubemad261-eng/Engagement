...
export default function Memories(){
 ...
 function backToCamera(){sessionStorage.setItem('returnToCamera','1');window.history.back()}
 return <main className="memory-page"><div className="memory-page-inner"><button type="button" className="memory-back" aria-label="Back to Camera" onClick={backToCamera}>← Back to Camera</button><p className="eyebrow">Memory wall</p><h1>Moments from<br/><em>our guests.</em></h1>{memories.length?<div className="memory-grid">{memories.map(m=><article key={m.id}><img src={m.image_url} alt="Guest memory"/><div className="memory-actions"><button onClick={()=>downloadMemory(m)}><Download size={14}/> Save</button>{session&&m.owner_id===session.user.id&&<button onClick={()=>deleteMemory(m)}><Trash2 size={14}/> Delete</button>}</div></article>)}</div>:<div className="story-placeholder"><Heart size={18}/><p>No guest memories yet. Be the first to share one.</p></div>}</div></main>
}
