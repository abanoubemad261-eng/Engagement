...
export default function Memories(){
 ...
 useEffect(()=>{const audio=audioRef.current;if(!audio)return;audio.volume=.75;const play=()=>audio.play().catch(()=>{});play();const unlock=()=>{play();window.removeEventListener('touchstart',unlock);window.removeEventListener('pointerdown',unlock)};window.addEventListener('touchstart',unlock,{passive:true});window.addEventListener('pointerdown',unlock,{passive:true});return()=>{window.removeEventListener('touchstart',unlock);window.removeEventListener('pointerdown',unlock)}},[])
 ...
 return <main className="memory-page"><audio ref={audioRef} src="/Ed_Sheeran_-_Perfect(256k).mp3" loop preload="auto" playsInline/><div className="memory-page-inner"><a className="memory-back" href="/#camera" aria-label="Back to Camera"><ArrowLeft size={14}/> Back to Camera</a><p className="eyebrow">Memory wall</p><h1>Moments from<br/><em>our guests.</em></h1>{memories.length?<div className="memory-grid">{memories.map(m=><article key={m.id}><img src={m.image_url} alt="Guest memory"/><div className="memory-actions"><button onClick={()=>downloadMemory(m)}><Download size={14}/> Save</button>{session&&m.owner_id===session.user.id&&<button onClick={()=>deleteMemory(m)}><Trash2 size={14}/> Delete</button>}</div></article>)}</div>:<div className="story-placeholder"><Heart size={18}/><p>No guest memories yet. Be the first to share one.</p></div>}</div></main>
}
