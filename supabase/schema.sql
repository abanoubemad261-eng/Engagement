...
-- Shared wishes: everyone can read and submit; only authenticated admin can delete.
...
-- RSVP: guests can submit; authenticated admin can read/delete.
...
-- Story photos: public read; authenticated admin manage.
...
-- Memory photos: public read; anonymous/authenticated owner can insert/delete only their own row.
drop policy if exists "public can read memory photos" on public.memory_photos;
drop policy if exists "public can add memory photos" on public.memory_photos;
drop policy if exists "authenticated can manage memory photos" on public.memory_photos;
...
create policy "admin can manage memory photos" on public.memory_photos for all to authenticated using (true) with check (true);

-- Settings: public read; authenticated admin manage.
...
-- Storage: public read for website rendering.
...
-- Admin uploads/deletes story and frame assets.
...
-- Guest camera uploads use a folder named with the anonymous auth user's UUID.
...
create policy "admin delete memory media" on storage.objects for delete to authenticated using (bucket_id='memory-photos');
