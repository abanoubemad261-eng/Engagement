...
create table if not exists public.story_photos (id uuid primary key default gen_random_uuid(),image_url text not null,storage_path text,sort_order integer not null default 0,created_at timestamptz not null default now(),story_date date,story_time time,caption text);
...
alter table public.story_photos add column if not exists story_date date;
alter table public.story_photos add column if not exists story_time time;
alter table public.story_photos add column if not exists caption text;
